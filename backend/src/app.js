...
// Serves the admin panel (frontend/) at /admin so this backend can host it
// directly — the default, same-origin deployment. When frontend/ is deployed
// separately (e.g. Vercel), this mount is simply unused there; see
// frontend/js/config.js and ALLOWED_ORIGIN above for the split-origin path.
app.use('/admin', express.static(path.join(__dirname, '../../frontend')));
// Runtime-written uploads (hospital logos) live under backend/, not inside the
// git-tracked frontend/ source tree — served separately so the URL shape
// (/admin/uploads/logos/<file>) is unchanged from before this reorganization.
